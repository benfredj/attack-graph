...
{
// member variables
private long numerator, denominator;  // stores the fraction data
...
public Fraction2( long numerator, long denominator ) {
   if (denominator == 0)
...
public Fraction2( long numerator ) {
	   this.numerator = numerator;
	   this.denominator = 1;
	   this.reduce();
	}
...
public boolean equals (Fraction2 fraction){
...
**********************************************************/
public long length(){
   long count = 0; // at least 3 for num, denom, and /
   boolean negative = false;
   //if (this.numerator<0) negative = true;
   String buffer = Long.toString(this.numerator);
   count +=buffer.length();
   if (this.denominator != 1) {
      buffer = Long.toString(this.denominator);
      count +=buffer.length();
      count++; //acount for /
   }
   return count;
...
public Fraction2 add(Fraction2 b)
{
   ...
   Fraction2 commonA = new Fraction2();
   Fraction2 commonB = new Fraction2();
   commonA = convert(common);
   commonB = b.convert(common);
   // create new fraction to return as sum
   Fraction2 sum = new Fraction2();
   // calculate sum
...
/**********************************************************
 ...
 Returns:        A Fraction representing the differenct of the
                 two fractions a & b
***********************************************************/
public Fraction2 subtract(Fraction2 b)
{
   // check preconditions
   if ((denominator == 0) || (b.denominator == 0))
      throw new IllegalArgumentException("invalid denominator");
...
/**********************************************************
 ...
 Returns:        A Fraction representing the product of the
                 two fractions a & b
***********************************************************/
public Fraction2 multiply(Fraction2 b)
{
   // check preconditions
   if ((denominator == 0) || (b.denominator == 0))
      throw new IllegalArgumentException("invalid denominator");
   // create new fraction to return as product
   Fraction2 product = new Fraction2();
   // calculate product
...
public Fraction2 inverse(Fraction2 b){
   if ((denominator == 0) || (b.numerator == 0))
      throw new IllegalArgumentException("invalid denominator");
   Fraction2 result = new Fraction2();
...
public String toString()
{
   ...
   else buffer = Long.toString(numerator);
   return buffer;
}
...
*/
private long lcd(long x1,long x2) {
    if(x1<=0 || x2<=0) {
...
*/
private long gcd(long x1,long x2) {
    if(x1<0 || x2<0) {
...
***********************************************************/
private Fraction2 convert(long common)
{
   Fraction2 result = new Fraction2();
